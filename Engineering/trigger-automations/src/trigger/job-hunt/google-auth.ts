...
// One refresh token covers both scopes (gmail.send + spreadsheets).
 ...
 * long-running scrape run doesn't die mid-request with a stale token.
 */
export async function getAccessToken(): Promise<string> {
  const now = Date.now();
  if (cache && cache.expiresAtMs - 60_000 > now) return cache.accessToken;

  // Dedicated Desktop-type OAuth client for job-hunt (separate from meetsync).
  // Scopes on the refresh token: gmail.send + spreadsheets.
