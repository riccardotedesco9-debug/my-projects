...
1. Embed design tokens inline (for `file://` compatibility)
   ```bash
   node .opencode/skills/design-system/scripts/embed-tokens.cjs --minimal
   ```
...
```bash
python .opencode/skills/design-system/scripts/search-slides.py "[goal]" -d strategy
```

### 3. Contextual Search Per Slide
```bash
python .opencode/skills/design-system/scripts/search-slides.py "[slide goal]" \
...
**Data location:** `.opencode/skills/design-system/data/`
