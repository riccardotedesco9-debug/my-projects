...
2. Download JSON → save as `.opencode/secrets/google_client_secret.json` (project) or `~/.opencode/secrets/` (global)
3. Authenticate:
```bash
...
Config: `google_client_secret.json` in `.opencode/secrets/` (project or `~/.opencode/secrets/` global)
