...
- **ElevenLabs** (`mcp__elevenlabs__*`) — Comprehensive audio platform:
  ...
  - Use as **primary tool for voice + sound design** — voiceover/TTS, voice clone/design, audio processing, and *designed/standalone* SFX. **Music is NOT default-AI anymore** — primary music + SFX = **Epidemic Sound** (licensed library); fal/ElevenLabs music = the **AI-generated fill lane** only (ElevenLabs = quick in-tool bed, fal = richer AI-gen, vocals, video-to-music). See the Audio sourcing rule under "Visual & Audio / Creative Routing".
