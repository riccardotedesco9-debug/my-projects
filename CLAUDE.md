...
- **ElevenLabs** (`mcp__elevenlabs__*`) — Comprehensive audio platform:
  ...
  - Use as **primary tool for voice + sound design** — voiceover/TTS, voice clone/design, audio processing, and *designed/standalone* SFX. **fal hosts EL's *generators* (TTS/SFX/music) but the account-stateful niche ops are ElevenLabs-only:** voice **cloning**, voice **design** + public Voice Library, **speech-to-speech** conversion, audio **isolation/cleanup**, **STT + diarization**, conversational **voice agents** — so fal's `elevenlabs/music` is not a reason to drop ElevenLabs. **Music is NOT default-AI anymore** — primary music + SFX = **Epidemic Sound** (licensed library); fal/ElevenLabs music = the **AI-generated fill lane** only (ElevenLabs = quick in-tool bed, fal = richer AI-gen, vocals, video-to-music). See the Audio sourcing rule under "Visual & Audio / Creative Routing".
...
- **Music & SFX (commercial/legal)** → **Epidemic Sound** (primary licensed library; manual browse/drag via the native Resolve plugin); **personal/unlicensed** → Spotify via `spotdl` (preferred) or `yt-dlp` (backup); **AI-generated fill** → fal via `creative-router`; **voice/VO + designed SFX** → ElevenLabs — not code-based audio generation. See Audio sourcing below.
...
- **Audio sourcing (music + SFX — so there's never a "which one?"), in priority order (updated 2026-06-29):** (1) **music + SFX for legal/commercial/client work → Epidemic Sound** — licensed all-genre library with stems; you browse + drag via the native **DaVinci Resolve "Soundtracking" plugin** (or site download). This is a **manual, you-logged-in** step — there is **no usable Epidemic API** (Partner API is B2B-gated/being withdrawn; never automate the account); Claude picks up the file the plugin drops in the Resolve **project folder** and works it downstream. (2) **personal / licensing-don't-care videos → Spotify via `spotdl`** (preferred over YouTube; lossy, personal-use only), **`yt-dlp`** as backup — both invoked `python -m spotdl` / `python -m yt_dlp`. (3) **AI-generated *fill* → fal via `creative-router`** (`stable-audio-3`, vocals `minimax-music`/`lyria3`, video-to-music `sonilo`) when no human/library track fits — **no longer the default**. (4) **voice / voiceover / designed SFX → ElevenLabs.** (5) an AI-**generated** video → the renderer's **native audio**, never bolt ElevenLabs or fal music on top. All lanes funnel into DaVinci via `projects/video-pipeline` (`sync.py`).
...
- **AI video & audio (learned 2026-06-06):** for any AI-generated video, take the **sound from the same renderer** — the fal video model's **native audio** (`generate_audio`, e.g. Seedance/Kling/Veo/Sora). **Do NOT layer ElevenLabs SFX onto AI video** — it desyncs and sounds tacky. ElevenLabs is for **standalone voice + SFX** (voiceover, UI/game SFX) or audio over **stills/edits you fully control**; **standalone music routes per Audio sourcing above** — Epidemic (commercial) / Spotify `spotdl` (personal) / fal `creative-router` (AI-gen fill). Live-verify model picks via `recommend_model`; observed: **Seedance 2** = best product realism + native audio; **Kling v3** drifts/hallucinates past ~10–15s; **Sora 2** = 9:16/16:9 only. AI video renders ~24fps → smooth to 60fps with `ffmpeg minterpolate` (don't just `-r` it — that judders). Anti-hallucination: seed from a real image and **feed real product/packaging refs** (or it invents them), soft motion verbs, ≤8s, in-prompt negatives.
