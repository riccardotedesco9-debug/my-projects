...
function sectionViewer_(S, env) {
  ...
  S.t('viewer: sheet named Hours', scratch.getSheets()[0].getName(), 'Hours');
  ...
  var totals = sh.getRange('B6').getFormula();
  var sessions = sh.getRange('F6').getFormula();
  S.t('viewer: totals QUERY is IFERROR-wrapped (empty log ≠ access error)', totals.indexOf('=IFERROR(QUERY(IMPORTRANGE') === 0, true);
  ...
  var probe = (totals + sessions).split(env.ss.getId()).join('');
  var moneyLeak = /A2:[GH]|Col7|Col8/.test(probe);
  S.t('viewer: imports A2:F only — Rate/€ can never leak', totals.indexOf('A2:F') > 0 && sessions.indexOf('A2:F') > 0 && !moneyLeak, true);
