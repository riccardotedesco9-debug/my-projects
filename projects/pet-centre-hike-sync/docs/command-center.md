...
| **Refresh charts + stock colours** | Build/refresh the charts ("Hike Insights" tab: pies on top, bars below, colour legend at the bottom) and re-apply the live stock colour-grading on the DATA SHEET's Stock on hand column — green → yellow → orange → red → dark red as stock falls vs each product's reorder level. |
